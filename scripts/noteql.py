...
engine = sqlalchemy.create_engine('postgres://noteql:noteql@db/noteql')
...
class Session:
    def __init__(self, schema, drop=False):
        self.schema = schema
...
def load_json_command_line(filename, path, table, field, append):
    load_json(filename, path, table, field, append)
