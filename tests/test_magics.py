...
def create_test_table():
    dfs = ip.run_cell_magic('nql', 'CREATE test', '''
...
def test_df_assign():
    ...
    assert ip.user_ns['df'].to_dict('list') == {'atitle': ['aa', 'aaa'], 'btitle': ['bb', 'bbb']}


def test_all_assign():
    create_test_table()

    ip.run_cell_magic('nql', 'col=COL cols=COLS row=ROW rows=ROWS cell=CELL record=RECORD records=RECORDS headings=HEADINGS sql=SQL', SIMPLE_QUERY)

    assert ip.user_ns['headings'] == ['atitle', 'btitle']

    assert ip.user_ns['records'] == [{'atitle': 'aa', 'btitle': 'bb'}, {'atitle': 'aaa', 'btitle': 'bbb'}]
...
def test_multi_sql():
    ...
    ip.run_cell_magic('nql', 'col=COL cols=COLS row=ROW rows=ROWS cell=CELL record=RECORD records=RECORDS headings=HEADINGS sql=SQL',
    ...
    assert ip.user_ns['records2'] == [{'atitle': 'aa', 'btitle': 'bb'}, {'atitle': 'aaa', 'btitle': 'bbb'}]
