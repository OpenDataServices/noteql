...
def load_xml_command_line(file_name, tag, table, schema, field, dburi, append, overwrite):
    if not schema:
        schema = 'public'
    session = noteql.Session(schema, dburi=dburi)
    session.load_xml(file_name, tag=tag, table_name=table, field_name=field, append=append, overwrite=overwrite)
